...
- [Link](https://www.youtube.com/watch?v=rb-TXIlH_Io)
...
Launch the attached VM. From the AttackBox terminal, connect using Telnet to 10.10.78.227 80 and retrieve the file flag.thm. What does it contain?
...
- A ```STAT``` command can provide some added information. 
...
#### Pentester Terminal
```
pentester@TryHackMe$ telnet 10.10.78.227 21
Trying 10.10.78.227...
Connected to MACHINE_IP.
Escape character is '^]'.
```
...
![img](/assets/img/protocols-servers04.png)
...
#### Pentester Terminal
```
pentester@TryHackMe$ ftp 10.10.78.227
Connected to 10.10.78.227.
220 (vsFTPd 3.0.3)
Name: frank
331 Please specify the password.
Password: D2xc9CgD
...
![img](/assets/img/protocols-servers06.png)
...
We issue ```<CR><LF>.<CR><LF>``` (or ```Enter . Enter``` to put it in simpler terms). The SMTP server now queues the message.
...
![img](/assets/img/protocols-servers08.png)
...
Connect to the VM (10.10.78.227) at the POP3 port. Authenticate using the username frank and password D2xc9CgD. What is the response you get to STAT?
![img](/assets/img/protocols-servers09.png)
```
Answer : +OK 0 0
```
How many email messages are available to download via POP3 on 10.10.78.227?
```
Answer : 0
```
---
...
1. In the console output below, we use ```Telnet``` to connect to the IMAP server’s default port
2. Then we authenticate using ```LOGIN username password```. 
